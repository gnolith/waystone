...
Gnolith packages are versioned independently. The current line supports optional `@gnolith/taproot >=0.3.0 <0.5.0`. The 0.3 floor is required by mandatory authored statement text. The range is widened through 0.4 because Waystone owns structural browser protocols and imports no Taproot runtime; a host must still qualify its exact Taproot and Site route artifacts. Changes to the range require exact-package isolated-consumer verification.
