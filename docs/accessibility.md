...
The unified search workflow manages focus on results and errors, exposes all kind/filter/cursor controls to the keyboard, and announces loading and lexical/semantic readiness. Automated axe coverage includes all seven result kinds, Resource/Annotation/Prompt content, health/admin controls, and host-operation progress. Confirmations use native controls and never substitute for server authorization.
